...
function stageCosts(stage,id=selectedProjectId){
 const labor=projectLabor(id).filter(x=>x.stage===stage).reduce((a,x)=>a+x.total,0);
 ...
 return{labor,materials,equipment,other,subcontractPaid,toPay,paid,committed:paid+toPay};
}
function projectTotals(id){const labor=projectLabor(id).reduce((a,x)=>a+x.total,0),items=projectItems(id).reduce((a,x)=>a+x.total,0),subcontracts=activeCommitments(id),subcontractPaid=subcontracts.filter(x=>paymentStatus(x.paymentStatus)==='Paid').reduce((a,x)=>a+x.amount,0),toPay=subcontracts.filter(x=>paymentStatus(x.paymentStatus)==='To Pay').reduce((a,x)=>a+x.amount,0),paid=labor+items+subcontractPaid,hours=projectLabor(id).reduce((a,x)=>a+x.hours,0);return{paid,toPay,committed:paid+toPay,hours}}
function projectAlerts(id){
 ...
 const groups=[['Compras sem classificação',unclassified],['Comprovantes sem link',noReceipt],['Totais de compra a conferir',mismatch],['Etapas a confirmar',stagesWaiting],['Pagamentos pendentes',payments]].filter(x=>x[1]>0);
...
function moneyLine(t){return `<div class="project-money"><span><small>Pago</small><b>${usd(t.paid)}</b></span><span><small>A pagar</small><b>${usd(t.toPay)}</b></span><span class="committed"><small>Custo comprometido</small><b>${usd(t.committed)}</b></span></div>`}
